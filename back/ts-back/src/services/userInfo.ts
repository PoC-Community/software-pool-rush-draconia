...
export const userInfo: RequestHandler = async (req, res) => {
  ...
  if (!req.query.email) return res.sendStatus(404);

  if (res.locals.user.type === "admin" || res.locals.user.email === req.body.email) {
    user = await getUser(req.query.email.toString());
  } else {
