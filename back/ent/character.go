...
func (*Character) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case character.ForeignKeys[0]: // user_characters
	...
	for i := range columns {
		...
		case character.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field user_characters", values[i])
			} else if value.Valid {
				c.user_characters = new(uuid.UUID)
				*c.user_characters = *value.S.(*uuid.UUID)
			}
		}
	}
	return nil
...
func (c *Character) QueryUser() *UserQuery {
	return NewCharacterClient(c.config).QueryUser(c)
}
